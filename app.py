...
if menu == "📝 Abrir Registro":
    ...
    with st.container(border=True):
        ...
        if tipo_reg == "Denúncia":
            st.info("🔒 **Sua denúncia é 100% anônima.** O síndico verá apenas o local e o relato.")
        
        cat_sel = st.selectbox("Área:", ["Corredor", "Garagem", "Jardim", "Academia", "Elevador", "Piscina", "Outros"])
        detalhe = st.text_input("Localização específica (ex: Bloco B, Vaga 12):")
        ...
        if st.button("ENVIAR REGISTRO", type="primary"):
            if desc:
                ...
                st.success(f"✅ Enviado com sucesso! Protocolo: **{prot}**")
...
# --- 3. PAINEL ADM ---
    ...
    if not st.session_state.auth_adm:
            ...
            for _, row in df.iterrows():
                ...
                with st.expander(f"{cor_emoji} {row['id']} - {row['tipo_registro']}"):
                    if row['status'] == "Pendente":
                        st.markdown('<p class="status-pendente">Aguardando Início (Pendente)</p>', unsafe_allow_html=True)
                    else:
                        st.markdown('<p class="status-manutencao">Em Execução (Manutenção)</p>', unsafe_allow_html=True)
                    
                    st.write(f"**Local:** {row['categoria']} ({row['local_detalhado']})")
                    st.write(f"**Relato:** {row['descricao']}")
                    if row['foto_path']: st.image(row['foto_path'])
                    
                    # CORREÇÃO AQUI: De ConUG para Finalizado
                    novo_st = st.selectbox("Status Mudar:", ["Pendente", "Em Manutenção", "Finalizado"], 
                                         index=["Pendente", "Em Manutenção", "Finalizado"].index(row['status']),
                                         key=f"adm_{row['id']}")
        ...
        with tab2:
            ...
            else: st.info("Ainda não há chamados finalizados.")
